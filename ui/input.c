...
InputEvent *qemu_input_event_new_btn(InputButton btn, bool down)
{
    ...
    evt->u.btn->down = down;
    /* 20151231 revised by chenbin. Init wheel as 0 */
    evt->u.btn->wheel = 0;
    return evt;
}
...
/* 20151231 added by chenbin. Add new update_button function with wheel */
static InputEvent *qemu_input_event_new_btn_wheel(InputButton btn, bool down, int32_t wheel)
{
    InputEvent *evt = g_new0(InputEvent, 1);
    evt->u.btn = g_new0(InputBtnEvent, 1);
    evt->type = INPUT_EVENT_KIND_BTN;
    evt->u.btn->button = btn;
    evt->u.btn->down = down;
    evt->u.btn->wheel = wheel;
    return evt;
}

static void qemu_input_queue_btn_wheel(QemuConsole *src, InputButton btn, bool down, int32_t wheel)
{
    InputEvent *evt;
    evt = qemu_input_event_new_btn_wheel(btn, down, wheel);
    qemu_input_event_send(src, evt);
    qapi_free_InputEvent(evt);
}

void qemu_input_update_buttons_wheel(QemuConsole *src, uint32_t *button_map,
                                    uint32_t button_old, uint32_t button_new,
                                    int32_t wheel)
{
    InputButton btn;
    ...
    for (btn = 0; btn < INPUT_BUTTON_MAX; btn++) {
        ...
        if (btn == INPUT_BUTTON_WHEEL_UP || btn == INPUT_BUTTON_WHEEL_DOWN) {
            qemu_input_queue_btn_wheel(src, btn, button_new & mask, wheel);
        } else {
            qemu_input_queue_btn(src, btn, button_new & mask);
        }
    }
