...
static int xhci_ep_nuke_one_xfer(XHCITransfer *t, TRBCCode report)
{
    ...
    if (t->running_async) {
        /** 20171221 suqixiong cancel packetä¹‹å‰å…ˆæ£€æŸ¥USBPacket->stateçš„å€¼
            è§£å†³D-Linkã€EDUPæ— çº¿ç½‘å¡å–æ¶ˆé‡å®šå‘æ—¶è™šæœºå´©æºƒé—®é¢˜ **/
        if (usb_packet_is_inflight(&t->packet)) {
            usb_cancel_packet(&t->packet);
        }
        /** 20171221 suqixiong end **/
        t->running_async = 0;
        killed = 1;
    }
    if (t->running_retry) {
                               ...
                               unsigned int epid)
{
    ...
    /* Fix blue screen error in win xp by xuyongjian */
    timer_del(epctx->kick_timer);
                             ...
                             unsigned int epid)
{
    ...
    if (!slot->uport) {
        printf("xhci_stop_ep : set to EP_ERROR state!\n");
        xhci_set_ep_state(xhci, epctx, NULL, EP_ERROR);
    } else {
        printf("xhci_stop_ep : set to EP_STOPPED state!\n");
                         ...
                         unsigned int epid, unsigned int streamid)
{
    ...
    while (1) {
        ...
        if (length < 0) {
            ...
            if (epctx->type == ET_ISO_IN && !epctx->tr_overrun) {
                XHCIEvent event = {ER_TRANSFER, CC_RING_OVERRUN};
                event.slotid = slotid;
                event.epid = epid;
                xhci_event(xhci, &event, epctx->last_intr);
                epctx->tr_overrun = 1;
            }
            break;
        }
        ...
        if (epctx->type == ET_ISO_IN && length > 0) {
            epctx->last_intr = TRB_INTR(xfer->trbs[0]);
            epctx->tr_overrun = 0;
        }
                                  ...
                                  uint64_t pictx, bool bsr)
{
    ...
    for (i = 0; i < xhci->numslots; i++) {
        ...
        if (xhci->slots[i].uport == uport) {
            printf("xhci: port %s already assigned to slot %d\n",
                    uport->path, i+1);
            return CC_TRB_ERROR;
        }
    }
...
static void xhci_port_reset(XHCIPort *port, bool warm_reset)
{
    ...
    if (!xhci_port_have_device(port)) {
        /** 20170314 suqixiong Èç¹û²»·µ»Øevent£¬xHCIÇý¶¯»á¿¨¶Ù5s£¬ÕâÆÚ¼äÈç¹û
            ²åÈëUSBÉè±¸£¬»á³öÏÖÎ´ÖªÉè±¸ÏÖÏó **/
        printf("%s, port %d, port have no device\n",
                __FUNCTION__, port->portnr);
                               ...
                               uint64_t val, unsigned size)
{
    ...
    case 0x18: /* ERDP low */
        ...
        erdp = xhci_addr64(intr->erdp_low, intr->erdp_high);
        dp_idx = (erdp - intr->er_start) / TRB_SIZE;
        if (intr->er_size != 0 && (intr->er_ep_idx + 1) % intr->er_size == dp_idx) {
            intr->er_ep_idx++;
            if (intr->er_ep_idx >= intr->er_size) {
                intr->er_ep_idx = 0;
                intr->er_pcs = !intr->er_pcs;
        ...
        if (intr->er_ep_idx != dp_idx && (val & ERDP_EHB)) {
            xhci_intr_raise(xhci, v);
        }
        break;
    case 0x1c: /* ERDP high */
