...
static void ehci_attach(USBPort *port)
{
    ...
    *portsc |= PORTSC_CSC;
    ...
    if ((*portsc & PORTSC_SUSPEND) && (*portsc & PORTSC_WKCN_E)) {
        s->need_resume |= (1 << port->index);
    }
...
static void ehci_detach(USBPort *port)
{
    ...
    *portsc |= PORTSC_CSC;
    ...
    /* XXX: check PORTSC_WKDS_E ???
     *      sometimes port can't resume if check it. */
                            ...
                            uint64_t val, unsigned size)
{
    ...
    *portsc &= val | ~PORTSC_PED;
    ...
    if ((val & PORTSC_PRESET) && !(*portsc & PORTSC_PRESET)) {
        ...
        /* Section 2.3.9: The host controller will unconditionally set
         * this bit to a zero when:
         * Software sets the Port Reset bit to a one (from a zero)
         */
        val &= ~PORTSC_SUSPEND;
        s->need_resume &= ~(1 << port);
    }
...
static void ehci_ports_resume(EHCIState *s)
{
    uint32_t need_resume_port;
    bool raise_irq = false;
    int i;

    for (i = 0; i < NB_PORTS; i++) {
        need_resume_port = 1 << i;
        if ( !(s->need_resume & need_resume_port) )
            continue;

        s->need_resume &= ~need_resume_port;
        if ( !(s->portsc[i] & PORTSC_SUSPEND) )
            continue;

        trace_usb_ehci_port_wakeup(i);
        s->portsc[i] |= PORTSC_FPRES;
        raise_irq = true;
    }
    if (raise_irq)
        ehci_raise_irq(s, USBSTS_PCD);
...
static void ehci_frame_timer(void *opaque)
{
    ...
    if (ehci_enabled(ehci) && ehci->need_resume) {
        ehci_ports_resume(ehci);
    }
